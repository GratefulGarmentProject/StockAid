...
Item.create([
              { description: "Women - Underwear - XS (5)", category_id: category_adult_underwear.id,
                current_quantity: random_numbers.sample, price: random_price },
...
def add_items(order, items)
  items.each do |item|
    order.order_details.build(quantity: [*1..item.current_quantity].sample, item_id: item.id, price: item.price)
  end
end
